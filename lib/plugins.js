 ...
 * Plugin options are exposed to client code in a JSON script tag.
 */
export function clientAssets(plugins) {
  ...
  if (body) head += `<script id="plugin-options" type="application/json">${JSON.stringify(clientOptions)}</script>\n`;
